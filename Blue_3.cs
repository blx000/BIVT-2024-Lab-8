...
namespace Lab_8
{
    public class Blue_3 : Blue
    {
        ...
        public override void Review()
        {
            ...
            //Извлечение первых букв всех слов
            ...
            foreach (char symb in tmp)
            {
                ...
                //Если буква не найдена, добавляем новую запись
                if (!fl) 
                {
            ...
            {
                if (!(x.Item1 == '\0')) //Пропускаем пустые ячейки
                {
            ...
            for (int i = 0; i < ans.Length - 1; i++)
            {
                for (int j = 0; j < ans.Length - i - 1; j++)
                {
                    ...
                    //Сначала сравниваем по процентам по убыванию
                    if (ans[j].Item2 < ans[j + 1].Item2)
                    {
                        swap = true;
                    }
                    //Если проценты равны, сравниваем по буквам по возрастанию
        ...
        public override string ToString()
        {
            ...
            return string.Join(Environment.NewLine, Array.ConvertAll(_output, cortege => $"{cortege.Item1} - {cortege.Item2:F4}")); //Форматирование вывода: буква - процент с 4 знаками после запятой
        }
    }
}